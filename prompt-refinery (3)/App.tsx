...
import { 
  ...
  ModelStrategy, BatchItem, BatchSummary, CustomAPIConfig, LLMProvider 
} from './types';
...
const App: React.FC = () => {
  ...
  const [customConfig, setCustomConfig] = useState<CustomAPIConfig>({
    ...
    authHeader: localStorage.getItem('custom_auth_header') || 'Authorization',
    authValue: localStorage.getItem('custom_auth_value') || '',
    requestFormat: (localStorage.getItem('custom_format') as any) || 'openai'
  });
  ...
  useEffect(() => {
    localStorage.setItem('custom_endpoint', customConfig.endpoint);
    localStorage.setItem('custom_model', customConfig.model);
    localStorage.setItem('custom_auth_header', customConfig.authHeader);
    localStorage.setItem('custom_auth_value', customConfig.authValue);
    localStorage.setItem('custom_format', customConfig.requestFormat);
  }, [customConfig]);
  ...
  const handleOptimize = async () => {
    ...
    setError(null);
    try {
      const res = await optimizePrompt(input, level, format, strategy, provider, apiKey, {
        useBetaModel,
        model: provider === LLMProvider.OPENAI ? 'gpt-4o-mini' : provider === LLMProvider.ANTHROPIC ? 'claude-3-5-sonnet-20241022' : 'deepseek-chat',
  ...
  const runBatchRefinery = async () => {
    ...
    for (const item of batchItems.filter(i => i.status === 'pending')) {
      ...
      try {
        const res = await optimizePrompt(item.original, level, format, strategy, provider, apiKey, {
          useBetaModel,
          model: provider === LLMProvider.OPENAI ? 'gpt-4o-mini' : provider === LLMProvider.ANTHROPIC ? 'claude-3-5-sonnet-20241022' : 'deepseek-chat',
          customConfig: provider === LLMProvider.CUSTOM ? customConfig : undefined
        });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20 selection:bg-indigo-500/30">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex bg-slate-800/50 p-1 rounded-xl border border-slate-700">
              ...
              <button onClick={() => setView('settings')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${view === 'settings' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}><ShieldCheck className="w-3.5 h-3.5" /> Settings</button>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        {view === 'business' ? (
          <MarketSheet provider={provider} apiKey={getActiveApiKey()} />
        ) : view === 'settings' ? (
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">LLM Provider Configuration</h2>
                  <p className="text-slate-400 text-sm">Connect your preferred AI provider and configure API access.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Google Gemini */}
                <div className={`p-6 rounded-2xl border transition-all ${provider === LLMProvider.GOOGLE_GEMINI ? 'bg-slate-950 border-indigo-500 ring-2 ring-indigo-500/30' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-200">Google Gemini</h3>
                      <p className="text-xs text-slate-500 mt-1">Fast & efficient reasoning</p>
                    </div>
                    <button onClick={() => setProvider(LLMProvider.GOOGLE_GEMINI)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${geminiKey ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                      {geminiKey ? 'Connected' : 'Not Set'}
                    </button>
                  </div>
                  <input placeholder="Enter Gemini API Key" type="password" value={geminiKey} onChange={(e) => setGeminiKey(e.target.value)} className="w-full mt-2 bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
                  <div className="mt-3 flex gap-2">
                    <input placeholder="Model (optional)" value="" onChange={() => {}} className="flex-1 bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-indigo-600 opacity-50 cursor-not-allowed" disabled />
                    <label className="flex items-center gap-2 text-xs text-slate-400 cursor-pointer mt-1">
                      <input type="checkbox" checked={useBetaModel && provider === LLMProvider.GOOGLE_GEMINI} onChange={(e) => { if (provider === LLMProvider.GOOGLE_GEMINI) setUseBetaModel(e.target.checked); }} className="rounded" />
                      Beta Model
                    </label>
                  </div>
                </div>
                ...
                {/* OpenAI */}
                <div className={`p-6 rounded-2xl border transition-all ${provider === LLMProvider.OPENAI ? 'bg-slate-950 border-emerald-500 ring-2 ring-emerald-500/30' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-200">OpenAI</h3>
                      <p className="text-xs text-slate-500 mt-1">GPT-4 & advanced models</p>
                    </div>
                    <button onClick={() => setProvider(LLMProvider.OPENAI)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${openaiKey ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                      {openaiKey ? 'Connected' : 'Not Set'}
                    </button>
                  </div>
                  <input placeholder="Enter OpenAI API Key (sk-...)" type="password" value={openaiKey} onChange={(e) => setOpenaiKey(e.target.value)} className="w-full mt-2 bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-emerald-600" />
                  <p className="text-[10px] text-slate-600 mt-2">Supports: GPT-4o, GPT-4 Turbo, etc.</p>
                </div>

                {/* Anthropic */}
                <div className={`p-6 rounded-2xl border transition-all ${provider === LLMProvider.ANTHROPIC ? 'bg-slate-950 border-purple-500 ring-2 ring-purple-500/30' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-200">Anthropic</h3>
                      <p className="text-xs text-slate-500 mt-1">Claude with safety focus</p>
                    </div>
                    <button onClick={() => setProvider(LLMProvider.ANTHROPIC)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${anthropicKey ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                      {anthropicKey ? 'Connected' : 'Not Set'}
                    </button>
                  </div>
                  <input placeholder="Enter Anthropic API Key" type="password" value={anthropicKey} onChange={(e) => setAnthropicKey(e.target.value)} className="w-full mt-2 bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-purple-600" />
                  <p className="text-[10px] text-slate-600 mt-2">Supports: Claude 3 Opus, Sonnet, Haiku</p>
                </div>

                {/* DeepSeek */}
                <div className={`p-6 rounded-2xl border transition-all ${provider === LLMProvider.DEEPSEEK ? 'bg-slate-950 border-cyan-500 ring-2 ring-cyan-500/30' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-200">DeepSeek</h3>
                      <p className="text-xs text-slate-500 mt-1">Cost-effective reasoning</p>
                    </div>
                    <button onClick={() => setProvider(LLMProvider.DEEPSEEK)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${deepseekKey ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                      {deepseekKey ? 'Connected' : 'Not Set'}
                    </button>
                  </div>
                  <input placeholder="Enter DeepSeek API Key" type="password" value={deepseekKey} onChange={(e) => setDeepseekKey(e.target.value)} className="w-full mt-2 bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-cyan-600" />
                  <p className="text-[10px] text-slate-600 mt-2">Supports: DeepSeek-V3, DeepSeek-Chat</p>
                </div>

                {/* Custom API */}
                <div className={`p-6 rounded-2xl border transition-all md:col-span-2 ${provider === LLMProvider.CUSTOM ? 'bg-slate-950 border-orange-500 ring-2 ring-orange-500/30' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-200">Custom API</h3>
                      <p className="text-xs text-slate-500 mt-1">Internal or third-party LLM endpoints</p>
                    </div>
                    <button onClick={() => setProvider(LLMProvider.CUSTOM)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${customConfig.endpoint ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                      {customConfig.endpoint ? 'Configured' : 'Not Set'}
                    </button>
                  </div>
                  <div className="space-y-3">
                    <input placeholder="API Endpoint URL" value={customConfig.endpoint} onChange={(e) => setCustomConfig({...customConfig, endpoint: e.target.value})} className="w-full bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-600" />
                    <div className="grid grid-cols-2 gap-3">
                      <input placeholder="Model Name" value={customConfig.model} onChange={(e) => setCustomConfig({...customConfig, model: e.target.value})} className="bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-600" />
                      <select value={customConfig.requestFormat} onChange={(e) => setCustomConfig({...customConfig, requestFormat: e.target.value as any})} className="bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-600">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <input placeholder="Auth Header" value={customConfig.authHeader} onChange={(e) => setCustomConfig({...customConfig, authHeader: e.target.value})} className="bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-600" />
                      <input placeholder="Auth Value" type="password" value={customConfig.authValue} onChange={(e) => setCustomConfig({...customConfig, authValue: e.target.value})} className="bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-600" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-blue-600/5 border border-blue-500/20 rounded-3xl p-8">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-2">
                  <h4 className="font-bold text-blue-400 text-sm uppercase tracking-widest">Multi-Provider Support</h4>
                  <p className="text-sm text-slate-400 leading-relaxed">
                    Configure multiple AI providers and switch between them using the provider selector. Each provider maintains its own API key and settings. Click a provider card above to activate it.
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="p-4 bg-slate-900/80 border-t border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                      ...
                      {provider === LLMProvider.GOOGLE_GEMINI && (
                        <div className="space-y-1">
                          <label className="text-[10px] text-slate-500 font-bold uppercase">Engine</label>
                          <div className="flex items-center justify-between bg-slate-950 border border-slate-800 rounded-lg p-2 h-[34px]">
                            <span className="text-[10px] text-slate-300 font-bold">{useBetaModel ? 'BETA' : 'FLASH'}</span>
                            <button 
                              onClick={() => setUseBetaModel(!useBetaModel)}
                              className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors focus:outline-none ${useBetaModel ? 'bg-amber-600' : 'bg-slate-700'}`}
                            >
                              <span className={`inline-block h-2.5 w-2.5 transform rounded-full bg-white transition-transform ${useBetaModel ? 'translate-x-4.5' : 'translate-x-1'}`} />
                            </button>
                          </div>
                        </div>
                      )}
